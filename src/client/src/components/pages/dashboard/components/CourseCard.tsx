...
export default function CourseCard({ 
  ...
  const hoverColor = variant === 'teacher' ? 'group-hover:text-indigo-600' : 'group-hover:text-blue-600';
    ...
    >
      ...
      <CardContent className="p-6 gap-2 flex flex-col">
        <div className="flex items-start justify-between">
          ...
          <IconChevronRight className={`w-5 h-5 text-muted-foreground ${hoverColor} group-hover:translate-x-1 transition-all`} />
        </div>
        ...
        <div className="flex mt-auto items-center justify-between pt-2 border-t border-gray-100">
          <div className="flex items-center space-x-4 text-xs text-muted-foreground">
            <div className="flex items-center space-x-1">
              ...
              <span>
                {(course as ICourse & { enrollmentCount?: number }).enrollmentCount || 0} {t('students').toLowerCase()}
              </span>
            </div>
            
            <div className="flex items-center space-x-1">
              <IconTarget className="w-4 h-4" />
              <span>{(course as ICourse & { topicCount?: number }).topicCount || 0} {t('topics').toLowerCase()}</span>
            </div>
          </div>
          
          <div className="flex items-center space-x-1 text-yellow-500">
            <IconStar className="w-4 h-4 fill-current" />
            <span className="text-xs font-medium">{variant === 'teacher' ? '4.9' : '4.8'}</span>
          </div>
        </div>
